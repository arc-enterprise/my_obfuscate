        ...
        def self.apply_table_config(row, table_config, columns)
            ...
            table_config.each do |column, definition|
                ...
                row[index.to_i] = case definition[:type]
                                      ...
                                      when :title
                                          clean_bad_whitespace(clean_quotes(FFaker::Lorem.sentence))
                                      when :blob
                                          clean_bad_whitespace(clean_quotes(FFaker::Lorem.sentences(5).join(".  ")))
                                      when :like_english
                                      ...
                                      when :dob
                                          File.open('obfuscate_log.txt', 'a') { |file| file.write("row: #{row[index]} \n") }
                                      ...
                                      when :street_address
                                          clean_bad_whitespace(clean_quotes(FFaker::AddressCA.street_address))
                                      when :secondary_address
                                          clean_bad_whitespace(clean_quotes(FFaker::AddressCA.secondary_address))
                                      when :city
                                          clean_quotes(FFaker::AddressCA.city)
                                      when :province
                                          clean_quotes FFaker::AddressCA.province_abbr
                                      when :postal_code
                                      ...
                                      when :job
                                          clean_bad_whitespace(clean_quotes(FFaker::Job.title))
                                      when :website
                                          clean_bad_whitespace(clean_quotes(FFaker::Internet.http_url))
                                      when :ipv4
                                      ...
                                      when :integer
                                          random_integer(definition[:between] || (10000..9999999)).to_s
                                      when :fixed
